...
CREATE OR REPLACE FUNCTION base_0_0_1.delete(criteria JSONB) RETURNS JSONB
AS $$
  --declare _crit JSONB;
    ...
    -- Validate Criteria]
      ...
      */
      ...
      BEGIN
          ...
          if criteria ? 'pk' and criteria ? 'sk' then
            -- [Delete where pk and sk]
            Delete from base_0_0_1.one
              where pk=criteria ->> 'pk' and sk=criteria ->> 'sk'
      ...
      EXCEPTION
          ...
          --  return format('{"status":"401", "msg":"Unauthorized", "criteria":%s}',criteria)::JSONB ;
          --when sqlstate 'PT403' then
          --  return format('{"status":"403", "msg":"Forbidden", "criteria":%s}',criteria)::JSONB ;
...
grant EXECUTE on FUNCTION base_0_0_1.delete(JSONB) to api_user;
